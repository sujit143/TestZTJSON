...
export class CommonHttpService {
  ...
  public globalGetService(url: string, data: any) {
    ...
    return this.http.get(url + querystring).toPromise().catch (
      e => {
  ...
  public globalGetServiceByUrl(url: string, data: any) {
    return this.http.get(url + data).toPromise().catch(
      e => {
        // console.log("error happend", e);
      }
    );
  ...
  public globalPostStreamService(url: string, data: any, header) {
    return this.http.post(url, data, header).toPromise().catch(
      e => {
        // console.log("error happend", e);
        if (e.status == 401) {
  ...
  HttpBlobPostService(url: string, data: any) {
    return this.AngHttp.post(url, data, {
  ...
  public getbyurl(url: string): Promise<any> {
    ...
    headers.append('Access-Control-Allow-Origin', '*');
    return this.http
      .get<any>(url, { headers: headers, responseType: 'json' as 'json' })
      .toPromise();
    // return file;
  }
}
