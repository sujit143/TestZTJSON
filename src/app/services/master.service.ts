...
export class MasterService {
  ...
  public loadStateList() {
    return this.httpService
      .globalGetServiceByUrl("assets/us_states_list.json", "")
      .then(data => {
        return data;
      });
  }
  public formatDataforDropdown(label, data, Placeholdervalue?, ValueField?) {
    ...
    _.forEach(data, function(value: any) {
      const shallow = _.clone(customdata);
      let alabel = '';
  ...
  sameasMappingData(form, mapobject, form2?) {
    ...
    _.forEach(mapobject, (value, key) => {
      ...
      if (form.get(key).value) {
        if (withtwoform == true) {
          form2.controls[value].patchValue(form.get(key).value, {
            emitEvent: true
          });
        } else {
          form.controls[value].patchValue(form.get(key).value, {
            emitEvent: true
          });
        }
      }
  ...
  public loadDateFormatList() {
    return this.httpService
      .globalGetServiceByUrl("assets/date_format.json", "")
      .then(data => {
        return data;
      });
  }
